...
export function HeroNeuralCanvas() {
  ...
  useEffect(() => {
    ...
    const drawBackdrop = (width: number, height: number, phase: number) => {
      ...
      const pointerX = pointer.active ? pointer.x * width : width * 0.82;
      const pointerY = pointer.active ? pointer.y * height : height * 0.22;
      const glowB = context.createRadialGradient(pointerX, pointerY, 0, pointerX, pointerY, width * 0.34);
      glowB.addColorStop(0, 'rgba(27,42,65,0.16)');
    ...
    const draw = (time: number) => {
      ...
      for (const particle of particles) {
        particle.x += isReducedMotion ? 0 : particle.vx + (pointer.active ? (pointer.x * width - particle.x) * 0.00002 : 0);
        particle.y += isReducedMotion ? 0 : particle.vy + (pointer.active ? (pointer.y * height - particle.y) * 0.000015 : 0);
      ...
      for (const particle of particles) {
        ...
        const glowBoost = pointer.active ? Math.max(0, 1 - pointerDistance / 180) : 0;

        context.beginPath();
        context.arc(particle.x, particle.y, particle.size + glowBoost * 1.3, 0, Math.PI * 2);
        context.fillStyle = `rgba(27, 42, 65, ${0.14 + glowBoost * 0.08})`;
        context.fill();

        context.beginPath();
        context.arc(particle.x, particle.y, particle.size * 0.42 + glowBoost * 0.4, 0, Math.PI * 2);
        context.fillStyle = `rgba(212, 175, 55, ${0.5 + glowBoost * 0.2})`;
        context.fill();
      }
